...
class aux():
    ...
    def create_widgets(self):
        ...
        # Campo de entrada (alargado)
        self.scan_entry = tk.Entry(self.root, width=125)
        self.scan_entry.grid(row=1, column=0, padx=10, pady=(0, 10), sticky="w")
        self.scan_entry.focus()
        self.scan_entry.bind("<Return>", self.search)

        # Botón de búsqueda
        self.button_search = tk.Button(self.root, text="Buscar", command=self.search)
        self.button_search.grid(row=1, column=1, padx=10, pady=(0, 10), sticky="w")

        # Etiqueta de modo de búsqueda
        self.scan_label = tk.Label(self.root, text="Seleccione modo de búsqueda:")
        self.scan_label.grid(row=2, column=0, padx=10, pady=(0, 10), sticky="w")

        # Selección de método de búsqueda
        self.type_options = ttk.Combobox(self.root, values=["Razon Social", "Rut"], state="readonly")
        self.type_options.grid(row=3, column=0, padx=10, pady=(0, 10), sticky="w")

        # Etiqueta de error
        self.error_label = tk.Label(self.root, text="", fg="red")
        self.error_label.grid(row=4, column=0, columnspan=2, padx=10, pady=(0, 10), sticky="w")

        # Lista de resultados (alargada)
        self.list_aux = tk.Listbox(self.root, borderwidth=2, relief="ridge", width=140, height=20)
        self.list_aux.grid(row=5, column=0, columnspan=2, padx=10, pady=10, sticky="w")

        # Botones agrupados
        button_frame = tk.Frame(self.root)
        button_frame.grid(row=6, column=0, columnspan=2, padx=10, pady=10, sticky="w")
        ...
        self.button_exit.grid(row=7, column=0, columnspan=2, padx=20, pady=(10, 20), sticky="w")
